...
Training ran on one Alienware 16 Aurora (NVIDIA RTX 5060 Laptop GPU, 8\,GB VRAM; 32\,GB RAM), ${\approx}15$,h in total ($2.6$--$4.1$,h per model). If necessary, the project may extend the training budget to the full parameters.
...
Representations are probed at the single 256-dimensional $[\mathrm{REG}]$ decoder-query vector after each encoder and decoder block, avoiding the dimensionality mismatch of probing the full flattened patch sequence. Rather than isolated locked-vs-control point
