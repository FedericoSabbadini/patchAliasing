...
\section{Methodology}
At first the Chronos Bolt-tiny model available on the HuggingFace Hub is used to evaluate the presence of structural aliasing in the default configuration, with patch size $P=16$ and stride $S=16$. To evaluate the presence and impact of structural aliasing, the project will analyze a set of synthetic signals with varying frequency characteristics. At first the Chronos Bolt-tiny model is tested over simple sine waves with known frequency sistematically varieted between 2 Hz and 256 Hz with 1 Hz increments, those tests are used to evaluate the presence of strcuctural aliasing.

Stride and patch size are then systematically varied according to the value reported in \autoref{tab:patchStride} to explore the explicit impact of patch geometry on aliasing effects. Since variations in the values of stride and patch size require a new training of the model we have selected a restricted set of parameters for which the model is re-trained.

\input{tables/patchStride.tex}

To obtain a comprehensive understanding of the impact of stride and patch size on the presence of structural aliasing, we have selected a set of reduced stride and patch size values, and a set of increased stride and patch size values, to evaluate the possibility of mitigating the phenomenon. We have decided to re-train the model also for the default values of patch and stride, with the aim of having a baseline for comparison with the other configurations. Further possible training of the model with patch size fixed to 16 and stride 1 and/or 15 to evaluate the impact of small, and the most extreme, variation of the stride  on the presence of structural aliasing and performance of the model.
...
The training procedure is based over the information provided by the official documentation about Chronos using 10M data produced with TSMixup and 1M data produced by KernelSynth.\cite{ansariChronosLearningLanguage2024a}. 

--- DA SISTEMARE ---

Possible description of the number of epochs and other parameters of the training procedure.
