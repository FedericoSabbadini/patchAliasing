...
\textbf{H2:} when $S$ is changed while $P$ is fixed, the locations of any local degradation will move proportionally to $1/S$. An effect that remains tied to $P$ rather than $S$ will refute the proposed stride-lock
mechanism.

\textbf{H3:} a layer-wise increase in the MDL required to decode tone presence or frequency at stride-locked conditions will predict a paired deterioration in forecast amplitude recovery and WQL. If forecast degradation occurs without representational loss, or vice versa, the claim of an internal representation bottleneck will not be supported.
...
\begin{itemize}
    \item \textbf{Sine Wave Signals:} simple sine waves with known frequency systematically varied between 2 Hz and 256 Hz with 1 Hz increments, to assess the model's ability to capture and represent periodic components across a range of frequencies.
...
Representations are probed, through MDL probing \cite{voitaInformationTheoreticProbingMinimum2020}, at the single 256-dimensional $[\mathrm{REG}]$ decoder-query vector after each encoder and decoder block, avoiding the dimensionality mismatch of probing the full flattened patch sequence. Rather than isolated locked-vs-control point
comparisons, a dense per-frequency accuracy/codelength profile is obtained via a hierarchical binary band-classification probe swept across the operational spectrum, with non-redundant phase sampling $S_f=\min\{f_s/\gcd(f,f_s)-1,\,N\}$ in place of independently drawn random phases.

The locked band $f_k\pm\varepsilon$ is read off this profile and compared against pre-specified controls at $f_k\pm\delta$, $\delta\gg\varepsilon$, using online codelength and forecast amplitude recovery as the two metrics, against shuffled-label and random-init controls. A codelength increase localized to the locked band at a given layer supports H1 there; a corresponding drop in
amplitude recovery supports H3. See Pagani et al. \cite{paganiPreliminaryInsightsChronos2026} for the band-classification and phase-sampling procedure this adapts.
