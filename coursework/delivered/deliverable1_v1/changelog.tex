...
A \emph{Probing Methodology} section replaces the single sentence of the first submission, specifying: the 256-d $[\mathrm{REG}]$ vector probed after each encoder and decoder block plus two output-stage points; the prequential-MDL codelength metric with shuffled-label and random-initialisation controls; and a dense per-frequency accuracy profile with deterministic phase sampling. Probing was extended from decoder-only to include the encoder, so that the stage at which frequency information is lost can be localised. The pipeline was simplified and staged: clean sinusoids first, then the retrained $(P,S)$ grid, then the photovoltaic application.
 
\subsection{Patch stride experiments}
It is now stated that changing $P$ or $S$ requires retraining (the former resizes the patch-embedding block, the latter alters inter-patch dynamics). Table~1 reports the five retrained configurations; Table~2 compares all hyperparameters against the published Chronos recipe and isolates the departures. The Chronos-Bolt recipe is not publicly available, so the loop is a budget-matched adaptation of the Chronos-T5 recipe, with a uniform 100k-step budget across all configurations to avoid confounding.
 
Configurations with $S > P$ are excluded: they discard samples between patches, confounding structural aliasing with data loss. The symmetric design requested in the feedback is realised on the $P{=}S$ diagonal ($\{8,16,24\}$), the only axis where the default has admissible neighbours on both sides under $S \le P$.
