...
For exact alignment between the signal and the tokenization operator, the phase accumulation over one stride must correspond to an integer number of $2\pi$--periods $2\pi\frac{fS}{f_s}=2\pi c, \qquad c\in\mathbb{N}^{+}$.
...
the admissible phase-locking frequencies correspond to integer harmonics of $f_{\mathrm{grid}}$ that remain below the Nyquist limit.
...
Using Equation \eqref{eq:element_def},
...
Applying Equation \eqref{eq:general_periodicity} gives
...
\subsection{Sinusoidal Signals as a Special Case}

We now show that sinusoidal signals belong to the broader class defined above whenever their frequency lies in $\mathcal{F}_{\mathrm{lock}}$.
...
and substituting Equation \eqref{eq:flock_set} yields
...
belongs to the translationally invariant class of Equation \eqref{eq:general_periodicity}, and thus satisfies
