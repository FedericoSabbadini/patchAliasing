...
\subsection{Core Concept Taxonomy}
...
\subsection{Semantic Relations \& Properties}
...
\subsection{Knowledge-Driven Inference System (SWRL Rules)}
...
\subsection{Chronos Structural Aliasing Hypotheses}
...
{\color{red}The appropriate detection tolerance derives from the frequency resolution of the
context window of length $N_\mathrm{context}$ samples:
\begin{equation}
\delta f \approx \frac{f_s}{N_\mathrm{context}}.
\end{equation}
For Chronos-Bolt default context lengths (512--2048 samples), this gives
$\delta f \in [f_s/2048,\, f_s/512]$.
The tolerance fraction $\varepsilon = \delta f / f_p = S/N_\mathrm{context}$ must be
validated empirically; whether $\pm 10\%$ is wider or narrower than this resolution
depends on the specific $S$ and $N_\mathrm{context}$ chosen.}


\begin{itemize}
    \item \textbf{Primary Blind Spot (H1):}
    \begin{equation}
    \begin{aligned}
    ...
    %% FIXED: replaced hardcoded 0.9/1.1 with ε-parameterised bounds
    &\wedge \texttt{swrlb:divide}(?r, ?f_0, ?f_p) \wedge (?r \ge 1-\varepsilon) \wedge (?r \le 1+\varepsilon) \\
    &\longrightarrow \texttt{exhibitsAliasing}(?ps, \texttt{H1\_BlindSpot})
    \end{aligned}
    \end{equation}

    \item \textbf{Secondary Harmonic Blind Spot (H1b):}
    \begin{equation}
    \begin{aligned}
    \texttt{Signal}(?s) &\wedge \texttt{tokenizes}(?s, ?ps) \wedge \texttt{fundamentalFrequencyHz}(?s, ?f_0) \wedge \texttt{patchInducedFrequencyHz}(?ps, ?f_p) \\
    &\wedge \texttt{swrlb:divide}(?r, ?f_0, ?f_p) \wedge (?r \ge 2-\varepsilon) \wedge (?r \le 2+\varepsilon) \\
