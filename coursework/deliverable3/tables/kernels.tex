% [H] pinned this table to the point of \input, which is the end of the appendix, so it
% opened a fresh page and left three quarters of it blank. [!htb] lets it settle onto the
% preceding text page. Restore [H] to revert.
\begin{table}[!htb]
\small
\centering
\begin{tabular}{L{0.21\linewidth} L{0.38\linewidth} L{0.32\linewidth}}
\toprule
Kernel & Formula & Hyperparameters \\
\midrule
Constant &
$\kappa_{\mathrm{Const}}(x,x')=C$ &
$C=1$ \\
White Noise &
$\kappa_{\mathrm{White}}(x,x')=\sigma_n \cdot \mathbf{1}_{x=x'}$ &
$\sigma_n \in \{0.1,1\}$ \\
Linear &
$\kappa_{\mathrm{Lin}}(x,x')=\sigma_0^2+x\cdot x'$ &
$\sigma_0 \in \{0,1,10\}$ \\
RBF &
$\kappa_{\mathrm{RBF}}(x,x')=\exp\!\left(-\frac{\lVert x-x'\rVert^2}{2l^2}\right)$ &
$l \in \{0.1,1,10\}$ \\
Rational Quadratic &
$\kappa_{\mathrm{RQ}}(x,x')=\left(1+\frac{\lVert x-x'\rVert^2}{2\alpha}\right)^{-\alpha}$ &
$\alpha \in \{0.1,1,10\}$, $l=1$ \\
Periodic &
$\kappa_{\mathrm{Per}}(x,x')=\exp\!\left(-2\sin^2\!\left(\pi\frac{\lVert x-x'\rVert}{p/l_{\mathrm{syn}}}\right)\right)$ &
\begin{tabular}[t]{@{}l@{}}
$p \in \{24,48,96,168,336,672,$\\
$7,14,30,60,365,730,$\\
$4,26,52,4,6,12,4,40,10\}$
\end{tabular} \\
\bottomrule
\end{tabular}
\caption{Kernel bank $\mathcal{K}$ used by KernelSynth.}
\label{tab:kernelsynth_bank}
\end{table}