...
\item Prior sensitivity: every prior scale is multiplied by $0.5$, $1$ and $2$, except the baseline $\alpha_0$ of Model~B, centred on the data, and its Gamma shape $r$, which are held fixed. This gives the ladder $\{0.25,0.5,1.0\}$ of \autoref{sec:models}; every rung must converge, the rule probability may move by at most $0.10$, and the decision class must be the same at every rung.
