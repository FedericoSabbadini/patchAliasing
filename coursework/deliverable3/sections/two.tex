...
Using the two families in \autoref{eq:branches}, the phase-locking frequency class $\mathcal{F}_{\mathrm{lock}}$ is defined as
...
\\
&=
x\!\left[kS+m+\frac{P}{\gcd(P,S)}S\right]
\\
&=
x[kS+m+P]
\\
...
In this regime the two branches separate, since $f_s/S>f_s/P$, the stride comb is the sparser of the two.

The derivation of \autoref{eq:collapse_S} makes no use of the relation between $P$ and $S$, so at $f\in\mathcal{F}_{S}$ consecutive tokens are still exactly equal. 
...
representation rather than of the operator, and one that can be established
