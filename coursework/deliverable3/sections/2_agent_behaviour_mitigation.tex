...
Pagani et al.\autocite{paganiPreliminaryInsightsChronos2026} report that frequency content is recovered unevenly from Chronos representations, \autoref{sec:tokenisation} supplies a set of frequencies at which that effect is expected.

The arithmetic gives a list of frequencies at which a loss is possible; whether anything is actually lost there, and whether the loss follows the rule the arithmetic predicts, is an empirical question, and the rest of this report is about settling it. Because $\mathcal{F}_{\mathrm{lock}}$ of \autoref{eq:flock} holds for any model, three claims can be carried into the analysis, each stated so that a particular measurement refutes it, and a mitigation proposed alongside them.
