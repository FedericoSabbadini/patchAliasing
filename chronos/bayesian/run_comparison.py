...
From the repository root:
  .venv/Scripts/python.exe chronos/bayesian/run_comparison.py --task all
  .venv/Scripts/python.exe chronos/bayesian/run_comparison.py --task all --smoke
"""
import argparse
...
def main():
    ...
    parser.add_argument('--output', type=Path, default=Path(__file__).parent/'_run/comparison')
    args = parser.parse_args()
    torch.set_num_threads(args.threads)
    csv = Path(__file__).parents[1]/'data/dataset/Dataset-SolarTechLab.csv'
