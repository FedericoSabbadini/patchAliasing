...
def runTSMixup(p, seed, realization):
    ...
    phase = (p.get("inject") or {}).get("phase", 0.0)
    print(f"TSMixup     -> {Path(gen.__str__()).name}  shape={sig.shape} std={sig.std():.4f}")


def runKernelSynth(p, seed, realization):
    ...
    sig = gen.generate(); gen.save(sig); gen.plot(sig)
    phase = (p.get("inject") or {}).get("phase", 0.0)
    print(f"KernelSynth -> {Path(gen.__str__()).name}  shape={sig.shape} std={sig.std():.4f}")
    ...
    for cfg in configs:
        ...
        for r in range(n_real):
            ...
            if cfg["generator"] == "tsmixup":
                runTSMixup(cfg["params"], seed, rid)
            elif cfg["generator"] == "kernelsynth":
                runKernelSynth(cfg["params"], seed, rid)
            else:
