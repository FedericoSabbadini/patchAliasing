...
All effect estimates are obtained through a probabilistic model implemented in PyMC. A \emph{skeptical prior},
\[
\beta \sim \mathcal{N}(0,\,0.1^2),
\]
encodes prior skepticism about the aliasing effect size. An observation-noise prior,
\[
\sigma \sim \mathrm{HalfNormal}(0.0616),
\]
is calibrated from the empirical standard deviation of \texttt{Power\_W} during nighttime null intervals in the dataset, representing baseline instrument noise under zero-generation conditions.

Posterior distributions, 95\% credible intervals, and Bayes factors comparing the aliasing model against a frequency-flat null model are reported throughout.

\begin{itemize}
    \item discussion of possible mitigation strategies, including stride reduction.
\end{itemize}